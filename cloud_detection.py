...
def admin_detection(image, det_model = Detection_Model):
    ...
    (name, percentage) = parse_string(det_string)
...
def draw_on_frame(image, coordinates, drawing_string):
    ...
    cv2.putText(image, drawing_string, (x,y), cv2.FONT_HERSHEY_SIMPLEX, 1, colors[1] ) 
    return image

print('Connecting to WolkAbout IoT Platform and send camera sensor reading.')
...
PROJECT_DIR_PATH = '/home/jovan/Desktop/IoT_project/'
...
print("Initialising OpenCV objects\n")

PATH_TO_CLASSIFIER = PROJECT_DIR_PATH + 'haarcascade_frontalface_default.xml'
...
print("Initialising Tensorflow objects\n")

PATH_TO_CKPT = PROJECT_DIR_PATH + 'frozen_inference_graph.pb'
...
while True:
    ...
    if(incrementer == 0):
        human = human_detection(frame, face_cascade)
        human_det_report = human.detected
        coordinates = human.coordinates
        if(human_det_report == True):
            coordinates2 = increase_rectangle(coordinates, frame)
            ROI_frame = image_roi(frame, coordinates2)
            ...
            drunk_report = random.choice([True, False])
        else:
            drunk_report = False

    if(coordinates != None):
        frame = draw_on_frame(frame, coordinates, drawing_string)
    cv2.imshow('stream', frame)

    btn = cv2.waitKey(state)
    if btn & 0xFF == ord('q'):
        break
    elif btn & 0xFF == ord('p'):
        if(state == play):
